...
def get_experiment_config(args, default_config):
    if args.exp == "pd_arena":
        ...
        raise Exception(
            "Please set --exp to be one of ['pd_arena', 'al_harvest', 'clean_up', \
    ...
    for i in range(len(player_roles)):
        ...
        policies[f"agent_{i}"] = policy.PolicySpec(
            ...
            config={
                "model": {
                    "conv_filters": [[16, [8, 8], 1], [128, [sprite_x, sprite_y], 1]],
                },
            },
        )
        player_to_agent[f"player_{i}"] = f"agent_{i}"

    run_configs.multi_agent(
        policies=policies,
        policy_mapping_fn=(lambda agent_id, *args, **kwargs: player_to_agent[agent_id]),
    )
